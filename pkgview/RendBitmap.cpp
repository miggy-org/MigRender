...
CRendBitmap::CRendBitmap(void)
	: m_hbmp(NULL), m_pbits(NULL), m_hMutex(NULL), m_width(0), m_height(0), m_stop(false)
{
}
...
void CRendBitmap::SetStop(void)
{
	m_stop = true;
}

bool CRendBitmap::IsStopped(void)
{
	return m_stop;
}
...
bool CRendBitmap::DoLine(int y, const dword* pline)
{
	...
	{
		memcpy(m_pbits + y*m_width, pline, m_width*sizeof(unsigned int));
